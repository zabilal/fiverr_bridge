...
class FiverrMessaging:
    ...
    def update_headers_from_cookies(self):
        ...
        self.headers["csrf-token"] = self.cookies()["_fiverr_session_key"].strip('"')

    @staticmethod
    def from_cookies_and_headers(cookies: dict[str, str],
                                 headers: Optional[dict[str, str]]) -> "FiverrMessaging":
        fiverr = FiverrMessaging()
        fiverr.session.cookie_jar.update_cookies(cookies)

        if headers:
            fiverr.headers = headers
            fiverr.using_headers_from_user = True
        else:
            fiverr.headers = fallback_headers
            fiverr.update_headers_from_cookies()

        # Skip these headers, including them will result in events being received in a newer
        # format that we don't support
        # fiverr.headers.pop("x-li-query-map", None)
        # fiverr.headers.pop("x-li-query-accept", None)
        # fiverr.headers.pop("x-li-accept", None)
    ...
    async def login(self, email: str, password: str, new_session: bool = True):
        ...
        logging.log("I am logging inside login line 214 of linkedin.py")
    ...
    async def get_user_profile(self) -> UserProfileResponse:
        res = await self._get("/my_profile")
